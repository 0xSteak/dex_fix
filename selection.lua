...
local TotallyNotSelectionChanged : BindableEvent = ExplorerPanel:WaitForChild("TotallyNotSelectionChanged")
...
function ReturnSetting(set)
	...
	elseif set == "ClickSelect" then
		return Settings.ClickSelect
	elseif set == "SelBox" then
		return Settings.SelBox
	end
end
...
TotallyNotSelectionChanged.Event:Connect(function()
	local Selection = TotallyNotGetSelection:Invoke()
	if Settings.SelBox and Selection and #Selection > 0 then
		highlightObj(Selection[1])
	else
		clearHighlight()
	end
end)
